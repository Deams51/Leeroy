<?php
	...
	if(count($this->msg['infos_projet']) > 0)
	{
		...
		$proprietaire = $this->getModel()->estProprietaire($user->id, $this->msg['infos_projet']['id']);
				echo '<a class="btn btn-large btn-success" href="javascript:void(0);" onclick="javascript:introJs().start();">Aide</a>';		echo '<div id="intro31" data-step="1" data-intro="La page qui contient les informations sur le projet.">';
	...
	<table>
		<thead>
			...
			<th data-step="10" data-intro="L'état de la demande maintenant (en attente/ ok)">Etat de la demande</th>
		</thead>
		<tbody>
	...
	<?php 
		if($proprietaire)
		{
			echo '<a href="'.JRoute::_('index.php?option=com_gpr&task=services.gestion&view=services&id_projet='.$this->msg['infos_projet']['id']).'"					data-step="11" data-intro="Vous pouvez demander une souscription à un service à l\'administrateur.">
		...
		jQuery(document).ready(function (){
			...
			var mess_sup_proj = 
				'Vous êtes sur le point de supprimer le projet.<br />'+
					'<ul>'+
						'<li>Tous les services associés au projet seront arrêtés, et donc inaccessibles.</li>'+
			...
			var mess_quit_proj = 
				'Vous êtes sur le point de quitter le projet.<br />'+
					'<ul>'+
						...
						'<li>Vous ne pourrez plus accéder aux services liés au projet</li>'+
					'</ul>'
				'Êtes-vous sur de vouloir continuer ?';
