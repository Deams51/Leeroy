	...
	class gprModelgpr extends JModelItem
	{
		...
		public function creerProjet($nom, $description, $id_createur)
		{
			...
			else
			{
				...
				$query = "INSERT INTO `#__gpr_projets`(nom, description, date_creation)
								VALUES (".$db->quote($nom).",".$db->quote($description).", CURDATE())";
				$db->setQuery($query);
		...
		public function supprimerProjet($id_projet, $id_user)
		{
			...
			$query = "DELETE FROM `#__gpr_projets` WHERE id=".$id_projet;
			$db->setQuery($query);
		...
		public function ajouterDemandeSvn($description, $taille, $droits, $id_projet)
		{
			...
			$query = "INSERT INTO `#__gpr_svn` (`raison_demande`,`taille`,`droits`, `id_projet`, `etat`, `date`)
						VALUES (".$db->quote($description).",".$db->quote($taille).",".$db->quote($droits).",
							".$db->quote($id_projet).",".$db->quote("attente").", CURDATE())";
		...
		public function ajouterDemandeVps($description, $taille, $ram_mini, $ram_burst, $nb_cpu, $id_projet)
		{
			...
			$query = "INSERT INTO `#__gpr_vps` (`raison_demande`,`espace_disque`,`ram_min`, `ram_burst`, `nb_cpu`, `id_projet`, `etat`, `date`)
						VALUES (".$db->quote($description).",".$db->quote($taille).",
							".$db->quote($ram_mini).", ".$db->quote($ram_burst).",
							".$db->quote($nb_cpu).",".$db->quote($id_projet).",".$db->quote("attente").", CURDATE())";
