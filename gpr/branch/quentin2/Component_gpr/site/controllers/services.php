	...
	class gprControllerServices extends gprController
	{
		...
		public function gestion_admin()
		{
			...
			if(!$user->guest)
			{
				...
				{
					$view->setErreur("Vous devez n'êtes pas autorisé à voir cette page.");
		...
		public function gestion()
		{
			...
			if(!$user->guest)
			{
				...
				if($id_projet)
				{
					...
					if($model->estMembre($user->id, $id_projet))
					{
						...
						if($submit)
						{
							...
							if($svn)
							{
								...
								if($description_svn && $taille_svn && $droits_svn)
								{
									...
									{
										$view->setErreur("Erreur interne : ".$model->getErreur());
										$this->_display('erreur');
									}
								}
								else
								{
									$view->setErreur("Veuillez renseigner tous les champs");
					...
					{
						$view->setErreur("Vous n'êtes pas membre du projet.");
						$this->_display('acces_interdit');
					}
				}
				else
				{
					$view->setErreur("Aucun id de projet renseigné");
					$this->_display('erreur');
				}
			}
			else
		...
		public function accepter_demande()
		{
			...
			if(!$user->guest)
			{
				...
				if(true)
				{
					...
					{
						$view->setErreur("Demande introuvable");
						$this->_display('erreur');
					}
				}
				else
