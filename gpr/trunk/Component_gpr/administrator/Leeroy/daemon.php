...
';
	...
	if(!count($results)) 
	{
		echo 'Nothing to do => refresh quota and exiting...
';
		vps_refresh_all_quota_used();
...
';
			...
			case 'vps_delete':
				try
				{
					...
					clean_up_action($res,$object);
					// maintenant qu'on a terminé on delete l'entrée dans la table vps
					delete_row('#__gpr_vps','id',$service->{'id'});
				}
				catch(Exception $e)
				{
					echo $e->getMessage();
